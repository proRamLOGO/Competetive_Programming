...
int32_t main(int argc, char** argv) {
    ...
    while(t--) {
        ...
        ll digitwise = 0 ;
        vll v1, v2;
        FOR(i,0,n-1,2) digitwise += v[i], v1.emplace_back(v[i+1]-v[i]) ;
        FOR(i,1,n-1,2) v2.emplace_back(v[i]-v[1+i]) ;

        ll _sum = max(3,0ll,kadane<ll>(v1),kadane<ll>(v2)) ;

        print(digitwise+_sum) ;
